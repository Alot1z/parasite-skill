...
| `skill_tools_gc` | `status?`, `age_days?`, `keep?`, `ledger_age_days?`, `ledger_keep?`, `dry_run?` | prune stale registry artifacts (agent reports + audit ledger) by age/keep, with `ledger_age_days`/`ledger_keep` overriding retention for the ledger only (independent TTL, JS `--ledger-age`/`--ledger-keep` parity), or with `status: true` report the gc posture (policy incl. ledger sub-policy, last/next sweep, throttle, stale dry-run) without pruning — parity with the JS twin's `tools gc` / `tools gc --status` |
