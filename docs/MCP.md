...
| `doctor` | `dirs?` (JS twin); `—` (Python twin) | one-shot health check: registry load + spec validation + tool readiness + audit-baseline diff + project-config parse + gc posture (both twins); auto-sweeps stale artifacts first under an `auto: true` policy (interval-throttled sweeps are informational, not failures); exits 1 on the first failing check |
