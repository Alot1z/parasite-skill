...
`parasite-skill site build` generates the full static documentation site
from the project's own data — multi-page HTML at deep nested routes:

```bash
parasite-skill site build                 # -> ./public
parasite-skill site validate              # verify routes + links of a build
```

The generated site covers `/skills/<id>/`, `/tools/<id>/`, `/agents/<id>/`,
`/clients/<id>/`, `/mcp/`, `/hooks/`, `/architecture/`, `/configuration/`,
`/guides/`, `/reference/commands/`, and `/changelog/`, plus the wiki, docs,
and the product landing page. Every route is a physical directory with an
`index.html`, so deep links and refreshes work with no JavaScript. Each page
carries a markdown twin for LLM ingestion, and the build also emits
`llms.txt` + `llms-full.txt` (LLM-readable corpus), `site-index.json`
(client-side full-text search), `routes.json` (route manifest with link
validation), `data/index.json` (structured machine-readable indexes),
`sitemap.xml`, `robots.txt`, and a `404.html`. The generator is
zero-dependency (no framework) and reads the registry, wiki, references,
docs, and repo source map as the canonical source — every documented
capability links back to its source file.

## GitHub Pages distribution

The `GitHub Pages` workflow publishes on `main`:

- the full static docs site generated by `site build` (landing, features,
  architecture, configuration, guides, reference, skills, tools, agents,
  clients, MCP, hooks, changelog, wiki, LLM corpus);
